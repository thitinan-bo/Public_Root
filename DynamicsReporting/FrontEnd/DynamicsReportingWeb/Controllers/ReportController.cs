    ...
    public class ReportController : Controller
    {
        ...
        public async Task<IActionResult> Index(int page = 1, int pageSize = 10)
        {
            ...
            try
            {
                ...
                var reqUserGroup = new ReqUserGroupReport
                {
                    GroupID = userId ?? 0,
                    currentPage = page,
                    pageSize = pageSize
                ...
                var groupReports = await _apiService.GetReportByGroupIdAsync(reqUserGroup);

                return View(groupReports);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error loading   Reports for UserId {UserId}", userId);
